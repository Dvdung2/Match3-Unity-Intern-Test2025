...
public class Board
{
    ...
    public Board(Transform transform, GameSettings gameSettings)
    {
        ...
        m_root.localScale = new Vector3(gameSettings.BoardScale, gameSettings.BoardScale, 1f);
    ...
    internal void Fill()
    {
        ...
        List<NormalItem.eNormalType> spawnList = new List<NormalItem.eNormalType>();

        int typeCount = Enum.GetValues(typeof(NormalItem.eNormalType)).Length;
        
        for (int i = 0; i < typeCount; i++) {
            spawnList.Add((NormalItem.eNormalType)i);
            spawnList.Add((NormalItem.eNormalType)i);
            spawnList.Add((NormalItem.eNormalType)i);
        }

        while (spawnList.Count < totalCells) {
            NormalItem.eNormalType randomType = Utils.GetRandomNormalType();
            spawnList.Add(randomType);
            spawnList.Add(randomType);
            spawnList.Add(randomType);
        }

        for (int i = 0; i < spawnList.Count; i++) {
            NormalItem.eNormalType temp = spawnList[i];
            int randomIndex = UnityEngine.Random.Range(i, spawnList.Count);
    ...
    public Cell GetRandomOccupiedCell()
    {
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                if (!m_cells[x, y].IsEmpty) occupied.Add(m_cells[x, y]);
            }
        }
        if (occupied.Count > 0) return occupied[UnityEngine.Random.Range(0, occupied.Count)];
        return null;
    }

    public Cell GetBestCellForAutoplay()
    {
        return GetRandomOccupiedCell();
    }

    public Cell GetCellWithItemType(NormalItem.eNormalType type)
    {
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
